...
export default async function handler(req, res) {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, x-candidate-id, x-question-index, x-candidate-name');
  if (req.method === 'OPTIONS') return res.status(200).end();
  if (req.method !== 'POST') return res.status(405).json({ error: 'Method not allowed' });

  const candidateId = req.headers['x-candidate-id'];
  const questionIndex = req.headers['x-question-index'];
  const candidateName = (req.headers['x-candidate-name'] || 'unknown')
  ...
  const path = `${candidateName}_${candidateId.substring(0,8)}/soru_${parseInt(questionIndex)+1}.webm`;
  ...
  try {
    ...
    if (buffer.length < 1000) {
      return res.status(400).json({ error: 'Video too small, likely empty' });
    }

    console.log(`Uploading: ${path}, size: ${buffer.length} bytes`);
